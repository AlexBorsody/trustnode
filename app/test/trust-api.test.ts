...
test("trust APIs preserve capture tokens, scope, private responses and exact export bytes", async () => {
  ...
  let dbCode: string | null = null, expired = false, authUnavailable = false, calls = 0, tableQueries: string[] = [], parameters: Record<string, unknown> = {};
  const inputText = '{"captured": 1, "hash": "exact whitespace"}';
  try {
    globalThis.fetch = async (url, init) => {
      const req = new Request(url, init), path = new URL(req.url).pathname;
      if (path.endsWith("/user")) return new Response(JSON.stringify(expired || authUnavailable ? { message: "Auth fixture error" } : { id: run }), { status: authUnavailable ? 503 : expired ? 401 : 200 });
      calls++;
      ...
      tableQueries.push(req.url);
      return new Response(JSON.stringify(path.endsWith("/tn_pack_versions") ? [{ id: version }] : [{ id: run, state: "queued" }]));
    };
    ...
    const listed = await enqueue.GET(new Request(`https://example.org/api/trust/runs?template_version=${version}`));
    assert.equal(listed.status, 200); assert.equal((await listed.json()).limit, 20);
    assert(tableQueries.at(-1)?.includes("tn_graph_snapshots.template_version_id=eq."));
    assert.equal((await enqueue.GET(new Request(`https://example.org/api/trust/runs?template_version=${version}&offset=-1`))).status, 400);
    ...
    expired = false; authUnavailable = true; assert.equal((await enqueue.POST(post())).status, 503); assert.equal(calls, before);
